...
def apply_ring_weight(nside):
    ...
    for m in range(nrings):
        ...
        weights[m] = 4. * pi / npix * (1. + w[northring - 1])
...
def apply_full_weights(map_hp, setwgt, nside):
    ...
    for i in range(2 * nside):
        ...
        for j in range(qpix << 2):
            ...
            if i != 2 * nside - 1:
                pix_value = apply_weight(map_hp[psouth + j], wgt[vpix + rpix], setwgt)
...
def ftm_shape(L: int, sampling: str, nside: int = None) -> tuple[int, int]:
...
def f_shape(L: int = None, sampling: str = "healpix", nside:int = None) -> tuple[int]:
